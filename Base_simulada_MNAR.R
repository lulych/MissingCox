...
prop <- 0.1 #Missing data proportion
...
while (i < miss+1) { #Repeat until you reach no. of missing in each variable
  j <- sample(1:dim, 1) #Pick the IDs randomly
  if (is.na(NX2$X2[j]) == 'FALSE') {NX2$X2[j] <- NA #Check that it is not NA
...
r <- 1
while (r < miss+1) { #Repeat until you reach no. of missing in each variable
  j <- sample(1:dim, 1) #Pick the IDs randomly
  if (is.na(MenosX4$X4[j]) == 'FALSE') {MenosX4$X4[j] <- NA #Check that it is not NA
  r <- r+1}  #Keeps track of NA generated
}
